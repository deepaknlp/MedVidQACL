...
def train_and_test_transformer(datasets, labels, filepath, max_seq_length, num_features, learning_rate=1e-5, dense_dim=12, epochs=args.epochs):
    ...
    # Test on Set 1
    test1_preds = model.predict(datasets['test.json'])
...
report = train_and_test_transformer(vit_transformer_datasets, vit_transformer_labels, "./multimodal_tmp/transformer_vit_video_classifier", VIT_MAX_SEQ_LENGTH, VIT_NUM_FEATURES, args.transformer_learning_rate)


# Save report
with open('multilabel_video_report_13_labels.pickle', 'wb') as f:
...
# Load report
with open('multilabel_video_report_13_labels.pickle', 'rb') as f:
